...
var url=urls.conversationTranscriptURL+"/getTranscript";var conversationTranscriptMode=CHM.getChat()&&CHM.getChat().getRule()?CHM.getChat().getRule().conversationTranscriptMode:null;var data=prepareBaseBodyForAsyncChatDataRequest();data.businessUnitID=CHM.getBusinessUnitID();if(!CHM.getChat().isAsyncReconnect()||CHM.getChat().getCustMsgCnt()>0){data.excludeEngagementID=CHM.getChatID();}
...
var eventName=(isIE()&&getBrowserMajorVer()<=7)?'onmouseup':'onclick';var c2cTheme=this.c2cSpec.c2cTheme[name];div.innerHTML='<div '+(isIOS()?' tabindex="0"':'')+'>'+c2cTheme+'</div>';if(clickable){if(C2CM.IMAGETYPES.ready===name&&this._rule.asyncChat()){this.requestCustomerStatus();}
...
function prepareBaseBodyForAsyncChatDataRequest(){var data={"siteID":getSiteID(),"customerID":Inq.getCustID()};if(getPersistCustID()){data.persistentCustomerID=JSON.stringify(getPersistCustID());}
